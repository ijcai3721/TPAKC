...
def prop_error_func(x: torch.Tensor, a: torch.int) -> torch.Tensor:
    """
    Function used to calculate the proportional error.
    """
    x = torch.abs(x)
    return (a*x) / torch.sqrt(1 - x ** 2)
...
class AttentionalCopula(nn.Module):
    ...
    def loss(
        ...
        if self.fixed_permutation:
            ...
            # Change permutation????????????????? to be based on DTW?????????????????????????
        ...
        centers = torch.linspace(0, 1, self.resolution).to(att_value.device)  # Assuming U[0,1]
        distances = (pred_true_u[:, 1:, None] - centers) ** 2  # Squared distance for Gaussian kernel

        # Convert distances to weights using a Gaussian kernel.
        # sigma is a hyperparameter defining the width of the Gaussian kernel.
        sigma = 0.03  
        weights = torch.exp(-distances / (2 * sigma ** 2))

        # Normalize weights so that they sum to 1 across the kernel centers for each value.
        weights = weights / weights.sum(dim=-1, keepdim=True)

        # Compute the smoothed likelihood.
        logits = self.dist_extractors(att_value)[:, 1:, :]
        logprob = (weights * (math.log(self.resolution) + nn.functional.log_softmax(logits, dim=2))).sum(dim=-1)


        probabilities = nn.functional.softmax(logits, dim=2)

        # Instead of discrete bins, the center of mass of the weighted kernel probabilities can be taken as predictions.
        bin_centers = torch.linspace(0, 1, self.resolution).to(logits.device).view(1, 1, -1)
        predictions_continuous = torch.sum(probabilities * bin_centers, dim=-1)


        # Proportional Error
        level_s = 3
        prediction_portions = torch.sum(predictions_continuous, dim=1, keepdim=True)
        prediction_portions = predictions_continuous / prediction_portions
        true_portions = torch.sum(pred_true_u[:, 1:], dim=1, keepdim=True)
        true_portions = pred_true_u[:, 1:] / true_portions
        pae = prop_error_func(prediction_portions - true_portions, level_s)
        pae = pae.sum(dim=1)
        
        return -logprob.sum(axis=1)+pae
